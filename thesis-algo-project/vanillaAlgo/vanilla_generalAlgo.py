...
def vanillaModel(N = None, nb_neurons = None, nb_hidden = None,r = None, dt = None,
    ...
    prc = Input(shape=(1,), name = "prc_0") # price of asset 1
    info = Input(shape=(1,), name = "information_set_0") # log-transformed and centered price of asset 1

    inputs = [prc, info] # the entire set of inputs logically comprises both the price and info tensors
    ...
    for j in range(N+1):            
        if j < N:
            ...
            # we generate our delta-hedging parameter through the output for each iteration of our model
            ...
            strat_helper = layer(helper1) # we input the transformed price into the model, which will then output our parameter
            ...
            # we first compute the cost of delta-hedging on our underlying asset
            ...
            # we prepare the two input parameters that are to be used for the next iteration in our algorithm
    ...
    # the model is called, with the two inputs given, as well as the wealth function as an output parameter (that is to be minimi
